...
check_services() {
    ...
    if [ -f "/root/Xray/config.json" ]; then
        ...
        # 获取Xray端口
        XRAY_PORT=$(grep '"port":' /root/Xray/config.json | head -1 | awk '{print $2}' | tr -d ',')
        
        # 获取Xray协议
        XRAY_PROTOCOL=$(grep -A10 'inbounds' /root/Xray/config.json | grep '"protocol":' | head -1 | awk -F'"' '{print $4}')
    fi
    
    # 检查Hysteria2是否安装
    if [ -f "/etc/hysteria/config.yaml" ]; then
        HYSTERIA_INSTALLED=1
        
        # 获取Hysteria2端口
        HYSTERIA_PORT=$(grep 'listen:' /etc/hysteria/config.yaml | awk '{print $2}' | tr -d ':')
    fi
}
...
install_hysteria2() {
    ...
    read -p "请输入 Hysteria 端口 (默认随机端口 $RANDOM_PORT): " PORT
...
modify_port() {
    ...
    echo " 修改服务端口"
    echo "=============================================="
    echo -e "${NC}1. 修改 Xray 端口"
    echo "2. 修改 Hysteria2 端口"
    echo "3. 返回主菜单"
    echo -e "${YELLOW}=============================================="
    echo -e "${NC}"
    ...
    case $choice in
        1) modify_xray_port ;;
        2) modify_hysteria2_port ;;
        3) show_menu ;;
        *) echo -e "${RED}无效选择，请重新输入${NC}"; sleep 1; modify_port ;;
    esac
}
...
modify_xray_port() {
    ...
    # 获取当前端口
    OLD_PORT=$(grep '"port":' /root/Xray/config.json | head -1 | awk '{print $2}' | tr -d ',')
    
    echo -e "${YELLOW}当前 Xray 端口: $OLD_PORT${NC}"
    ...
    # 备份配置文件
    cp /root/Xray/config.json /root/Xray/config.json.bak
    
    # 更新配置文件
    sed -i "s/\"port\": $OLD_PORT,/\"port\": $NEW_PORT,/" /root/Xray/config.json
    ...
    if [[ "$OS_TYPE" != "alpine" ]]; then
        ...
        iptables -D INPUT -p udp --dport $OLD_PORT -j ACCEPT >/dev/null 2>&1
    ...
    green "请使用新的端口更新客户端配置"
    sleep 2
}

modify_hysteria2_port() {
    if [ ! -f "/etc/hysteria/config.yaml" ]; then
        red "未找到 Hysteria2 配置文件，请先安装 Hysteria2！"
        sleep 2
        return
    ...
    # 获取当前端口
    OLD_PORT=$(grep 'listen:' /etc/hysteria/config.yaml | awk '{print $2}' | tr -d ':')
    
    echo -e "${YELLOW}当前 Hysteria2 端口: $OLD_PORT${NC}"
    read -p "请输入新的监听端口: " NEW_PORT
    ...
    # 更新配置文件
    sed -i "s/listen: :$OLD_PORT/listen: :$NEW_PORT/" /etc/hysteria/config.yaml
...
show_menu() {
    ...
    if [ $XRAY_INSTALLED -eq 1 ]; then
        echo -e "${NC}Xray 状态: ${GREEN}已安装${NC} (协议: $XRAY_PROTOCOL, 端口: $XRAY_PORT)"
    else
        echo -e "${NC}Xray 状态: ${RED}未安装${NC}"
    fi
    
    if [ $HYSTERIA_INSTALLED -eq 1 ]; then
        echo -e "Hysteria2 状态: ${GREEN}已安装${NC} (端口: $HYSTERIA_PORT)"
    else
        echo -e "Hysteria2 状态: ${RED}未安装${NC}"
    fi
